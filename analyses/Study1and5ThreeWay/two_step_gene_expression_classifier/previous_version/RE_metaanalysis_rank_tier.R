...
#       RE_panel_candidates_tier12.txt (Tier 1+2 genes for modeling)
# ===============================
...
message(sprintf("[DEBUG Step 1] Loaded rows: A=%d, B=%d", nrow(A), nrow(B)))
...
write_csv(meta_df, "RE_meta_ranked.csv")
message("[DEBUG Step 2] Wrote RE_meta_ranked.csv")
...
write_csv(tier_df, "RE_tiers.csv")
message("[DEBUG Step 3] Wrote RE_tiers.csv")
...
write_lines(tier12_genes, "RE_panel_candidates_tier12.txt")
message(sprintf("[DEBUG Steps 3/5] Wrote RE_panel_candidates_tier12.txt (n=%d)", length(tier12_genes)))
